...
description: "Replicating Statistics Canada's The Daily using Claude Code alongside dedicated tools and a skills-based harness."
tags:
  - cansim
  - thedaily
  - llm
...
If you've spent any time asking LLMs to work with numbers, you know the classic failure mode: confident, plausible-sounding statistics with no grounding in reality. Earlier models were particularly bad at this, inventing figures without hesitation. Even current reasoning-tuned models will occasionally bullshit you on statistical outputs. That type of error is particularly risky and frustrating in a project like this because LLMs will confidently give you plausible-sounding numbers with no grounding in reality, which can be hard to catch without spending nontrivial effort on review. 

Early versions of The D-AI-LY had a number of such subtle issues slip through: fabricated auxiliary values to fill summary tables, decimal place errors on small percentages, even articles generated for periods that hadn't been released yet. The numbers looked plausible and internally consistent, but were wrong.

The D-AI-LY implements several safeguards as a kind of multi-layer internal validation system. Skills certainly help here, and are part of the solution, but we also benefit from the agents' ability to sequentially validate each step of the process by running scripts and manipulating file structures. We essentially have three components: provenance tracking and compliance throughout the article creation process, a reasoning/subjective layer to play data skeptic, and a code reproducibility section made available to the reader. 

_*Update 1/11/2026: *_ I actually found this to be the most interesting part of the project and wrote up a much longer piece on how the data validation here works in a [follow-up post here.](https://dshkol.com/posts/the-daily-lie-detector/) 

All data must be fetched through a single R script that validates freshness, continuity, and value ranges at fetch time. Every article declares a `verification_json` file in its frontmatter linking to the authoritative data source. The build process fails if this verification is missing or invalid—enforcement through tooling, not documentation. Every article must include a reproducibility section with working R code and references to the source CANSIM table. 

The reactive/subjective part is where Claude's ability to reason based on the requirements laid out in the skills helps catch errors. These include common error situations, internal/external validity checks, narrative alignment checks,plausibility checks, and so on.  

The idea is that collectively these safeguards will catch enough errors that the velocity of this system isn't brought down by its lack of accuracy. 
